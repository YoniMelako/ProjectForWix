...
class RegisterForm extends Component {
    ...
    handleInputChange(event) {
        ...
        if (event.target.name !== 'address' && event.target.name !== 'city' && event.target.name !== 'birthday') {
    ...
    */
    handleSubmit(e) {
        ...
        let RegisterSuccess
        clientModel.RegisterClient(FormData)
            .then((result) => {
                if (result.success) {
    ...
    */
    ...
    render() {
        ...
        if (this.state.RegisterSuccess === true) {
            message = <div className="registerSuccess" style={{ color: "green" }}>
                Welcome {this.state.newclient} Your Register success
        </div>
            
        }
        else if (this.state.RegisterSuccess === false) {
            message = <div className="registerError" style={{ color: "red" }}>
                            ...
                            <form >
                                <div className="form-row">
                                    <div className="form-group col-md-6">
                                        ...
                                        <input type="text" className="form-control" name="firstname" value={this.state.userInfo.firstname} onChange={event => this.handleInputChange(event)} id="inputfirstname4" placeholder="First name" />
                                        {this.displayValidationErrors('firstname')}
                                    </div>
                                    <div className="form-group col-md-6">
                                        <label htmlFor="inputlastname4">Last Name</label>
                                        <input type="text" className="form-control" name="lastname" value={this.state.userInfo.lastname} onChange={event => this.handleInputChange(event)} id="inputlastname4" placeholder="Last name" />
                                ...
                                <div className="form-row">
                                    <div className="form-group col-md-6">
                                        ...
                                        <input type="text" className="form-control" name="city" value={this.state.userInfo.city} onChange={event => this.handleInputChange(event)} id="inputCity" placeholder="Ramle" />
                                    </div>

                                    <div className="form-group col-md-6">
                                        <label htmlFor="inputaddress">Street</label>
                                        <input type="text" name="address" value={this.state.userInfo.address} onChange={event => this.handleInputChange(event)} className="form-control" id="inputaddress" placeholder="Ben Gurion" />
                                    </div>

                                </div>
                                <div className="form-group">
                                    <label htmlFor="inputbirthday">Bitrh day</label>
                                    <input type="date" className="form-control" name="birthday" value={this.state.userInfo.birthday} onChange={event => this.handleInputChange(event)} id="inputbirthday" />
                                </div>
                                <div className="form-group">
                                    <label htmlFor="inputId">ID</label>
                                    <input type="text" className="form-control" name="Id" value={this.state.userInfo.Id} onChange={event => this.handleInputChange(event)} id="inputId" />
                                    {this.displayValidationErrors('Id')}
                                </div>
                                <div className="form-group">
                                    <label htmlFor="inputpassword">Password</label>
                                    <input type="password" className="form-control " name="password" value={this.state.userInfo.password} onChange={event => this.handleInputChange(event)} id="inputpassword" />
                                    {this.displayValidationErrors('password')}
                                </div>

                              {message}
                                <button type="button" className="btn btn-primary" onClick={this.handleSubmit} disabled={!this.isFormValid()} >Sign in</button>
                            </form>
                        </div>
